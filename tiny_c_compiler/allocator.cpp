...
static int align_down(int offset, unsigned alignment)
{
    int magnitude, remainder;

    if (alignment == 0)
        return offset;

    magnitude = -offset;
    remainder = magnitude % (int)alignment;

    if (remainder != 0)
        magnitude += (int)alignment - remainder;
...
void Block::allocate(int &offset) const
{
	...
	if (symbol->offset == 0) {
	    ...
	    offset = align_down(offset, symbol->type().alignment());
...
void Function::allocate(int &offset) const
{
	...
	if (i < types.size()) {
	    offset -= types[i].size();
	    offset = align_down(offset, types[i].alignment());
	    symbols[i]->offset = offset;
	} else
	    break;
